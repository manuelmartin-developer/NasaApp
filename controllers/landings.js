    ...
    getAllLandings: async (req, res) => {
        ...
        if(Object.keys(queries) == 0){
            ...
            res.status(200).render('landings', {
                ...
                const data = await Landing.find({mass: { $gte: minimum_mass }});
                res.status(200).render('landings', {jsStringify, data, iss});
            } catch (error) {
            ...
            const data = await Landing.find({year: { $gte: Number(from), $lte: Number(to) }});
            res.status(200).render('landings', {jsStringify, data, iss});
            
        }else if(queries.from){
            const iss = await ISS.getISS();
            const from = await req.query.from;
            const data = await Landing.find({year: { $gte: Number(from) }});
            res.status(200).render('landings', {jsStringify, data, iss});
            ...
            const data = await Landing.find({year: { $lte: Number(to) }});
            res.status(200).render('landings', {jsStringify, data, iss});
